...
**Designer** (high experience): Does documentation and establishes injection contracts as needed. Gathers requirements, forms big picture story, makes architectural trade-offs explicit, progressively refines documentation as patterns emerge. Requires domain expertise and architectural vision. Revises documentation and establishes what refactoring now needs to be executed. They implement API contracts into documentation for it, and pass it to testing. They also escalate major issues, along with a resolution plan, to auditing. Being able to produce quality documentation which begins holistically and progressively unfolds is **the** essential skill for all designers.

**Auditor** (highest experience): Auditors are the 'Alarm' of DDD. They attempt to maximize consistency between documents, tests, and code. Nothing gets committed without the relevant auditor's approval. Auditors are always looking for and surface any detected inconsistencies between the parts of the system, whether documents, tests, or code. This means with enough audit rounds most bugs are found and fixed and overall code quality increases. They can be a PM with deep domain expertise who reviews the documentation, a designer reviewing tests, a tester reviewing implementation, or something else entirely; however, within their domain, they **are** the person who is responsible for the final quality on this commit. Cannot be fully automated - requires human judgment and experience.
