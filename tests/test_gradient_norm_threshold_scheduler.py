...
class TestStatistics:
    ...
    def test_statistics_updates_after_accumulation(self):
        ...
        model = create_simple_model()
        optimizer, scheduler = create_optimizer_with_schedule(model, gradient_norm_threshold=0.001)
...
class TestGradientAccumulation:
    ...
    def test_accumulates_gradients_across_draws(self):
        ...
        model = create_simple_model()
        optimizer, scheduler = create_optimizer_with_schedule(
            model, gradient_norm_threshold=0.001
        )
    ...
    def test_gradients_cleared_after_step(self):
        ...
        model = create_simple_model()
        optimizer, scheduler = create_optimizer_with_schedule(
            model, gradient_norm_threshold=100.0
        )

        perform_forward_backward(model, grad_scale=0.1)
        optimizer.step()  # Should step with high threshold
