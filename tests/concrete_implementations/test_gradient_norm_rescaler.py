...
class SpyOptimizer(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        all_params = [p for group in self.real_optimizer.param_groups for p in group["params"]]
        grads = [p.grad.clone() if p.grad is not None else None for p in all_params]
...
def gnr_distributed_worker(rank, world_size, test_config, output_dir, master_addr, master_port):
    ...
    try:
        ...
        optimizer_wrapper = OptimizerWrapperGNR(
            spy, distributed_mode=test_config["distributed_mode"]
        )
...
class TestConstructor:
    ...
    def test_constructor_accepts_distributed_mode(self):
        ...
        optimizer_wrapper = OptimizerWrapperGNR(optimizer, distributed_mode="replicated")
    ...
    def test_constructor_accepts_all_parameters(self):
        ...
        optimizer_wrapper = OptimizerWrapperGNR(optimizer, distributed_mode="sharded")
    ...
    def test_constructor_validates_distributed_mode_values(self):
        ...
        with pytest.raises(ValueError):
            OptimizerWrapperGNR(optimizer, distributed_mode="invalid")
...
class TestAlwaysSteps:
    ...
    def test_always_steps_immediately(self):
        ...
        # Bind schedule
        tsa.constant_schedule(optimizer_wrapper, value=1.0, schedule_target="target_gradient_norm")
...
class TestGradientRescaling:
    ...
    def test_rescales_to_target_norm_single_param(self):
        ...
        tsa.constant_schedule(optimizer_wrapper, value=5.0, schedule_target="target_gradient_norm")
    ...
    def test_rescales_to_target_norm_multiple_params(self):
        ...
        tsa.constant_schedule(optimizer_wrapper, value=10.0, schedule_target="target_gradient_norm")
    ...
    def test_rescaling_formula_exact(self):
        ...
        tsa.constant_schedule(optimizer_wrapper, value=14.0, schedule_target="target_gradient_norm")
...
class TestDistributedMode:
    ...
    def test_replicated_mode_no_change_in_norm(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            tsa.constant_schedule(
                optimizer_wrapper_normal, value=5.0, schedule_target="target_gradient_norm"
            )
            ...
            assert math.isclose(
                logs[0][0]["captured_norm"], spy.get_last_captured_norm(), rel_tol=0.01
            )

    @pytest.mark.distributed
    @pytest.mark.skipif(sys.platform == "win32", reason="gloo not supported on Windows")
    def test_sharded_mode_combines_norms_for_rescaling(self):
        """Sharded mode merges norms using sqrt(sum(norm^2)/world_size) and always steps,
        rescaling to target."""
        world_size = 2
        ...
        # Both always step (not a threshold system)
        test_config = {"gradients": [3.0, 6.0], "target_norm": 5.0, "distributed_mode": "sharded"}
